...
export default function Praiagrande() {
  ...
  useEffect(()=>{
      async function previsao(){
        const Response = await api.get('weather?array_limit=10&fields=only_results,temp,city_name,date,time,forecast,max,min,wind_speedy,description&key=e4c586c4bd&city_name=PraiaGrande,SP');
    ...
    <View style={styles.bloco}>
        <Text>Cidade de Praia Grande</Text>
        <FlatList
        data={dados}
        renderItem={({item})=>{
          return(
