...
import api from './Api'
export default function Intanhaem() {
  const [dados, setDados] = useState("");
  useEffect(()=>{
      async function previsao(){
        const Response = await api.get('weather?array_limit=10&fields=only_results,temp,city_name,date,time,forecast,max,min,wind_speedy,description&key=e4c586c4bd&city_name=Intanhaem,SP');
